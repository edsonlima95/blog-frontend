    ...
    if (numOfPages.length > 1) {
        ...
        return (
            ...
            <>
                ...
                {current_page > (1 + siblingCount) && (<button onClick={() => onPageLink(first_page)}>Primeira</button>)}

                {previous.length > 0 && previous.map(page => {
                    return <button key={page} disabled={current_page === page ? true : false} onClick={() => onPageLink(page)}>{page}</button>
                })}

                <button disabled={current_page === current_page ? true : false} onClick={() => onPageLink(current_page)}>{current_page}</button>

                {next.length > 0 && next.map(page => {
                    return <button key={page} disabled={current_page === page ? true : false} onClick={() => onPageLink(page)}>{page}</button>
                })}

                {(current_page + siblingCount) < pages && (<button onClick={() => onPageLink(last_page)}>Última</button>)}
